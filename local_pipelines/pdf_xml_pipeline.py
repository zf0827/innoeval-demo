...
DEFAULT_INPUT_PATH = REPO_ROOT / "runs" / "task1_search_result.json"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "runs" / "pdf_xml_pipeline"
...
from local_pipelines.vendor.pdf_extraction.grobid_client import post_pdf
from local_pipelines.vendor.pdf_extraction.models import ExtractedPdfDocument, PdfReference
...
def build_run_dir(output_root: Path, result_tag: str | None) -> Path:
    tag = result_tag or datetime.now().strftime("%Y%m%d_%H%M%S_pdf_xml_pipeline")
...
class PdfXmlPipeline:
    ...
    def resolve_openalex(self, record: dict[str, Any], trace: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        append_trace(
            ...
            status="running",
            message="Skipping local KG title lookup in the open-source demo.",
