...
DEFAULT_MANIFEST_PATH = PROJECT_ROOT / "runs" / "pdf_xml_pipeline" / "manifest.json"
DEFAULT_TARGET_DIR = Path("/tmp/innoeval_grounding/target")
DEFAULT_RESULT_DIR = Path("/tmp/innoeval_grounding/result")
DEFAULT_EMBEDDING_MODEL_PATH = PROJECT_ROOT / "hf-models" / "BAAI--bge-large-en-v1.5"
DEFAULT_RERANKER_MODEL_PATH = PROJECT_ROOT / "hf-models" / "BAAI--bge-reranker-large"
...
from local_pipelines.utils import extract_text_from_pdf
from local_pipelines.vendor.pdf_extraction.parser import parse_tei_document
...
class QueryGeneratorConfig:
    api_url: str = DMX_API_URL
    model: str = DEFAULT_DMX_MODEL
...
class ParagraphReranker:
    def __init__(
        ...
        *,
        ...
        if device:
            kwargs["device"] = device
        self.model = CrossEncoder(str(model_path), **kwargs)
        self.batch_size = batch_size
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "search/result/pdf_xml_pipeline."
        )
    )
    ...
    parser.add_argument("--manifest", default=str(DEFAULT_MANIFEST_PATH), help="Manifest JSON from pdf_xml_pipeline.")
    parser.add_argument(
        "--papers-root",
        default=None,
        help="Optional override for the pdf_xml_pipeline run directory. Defaults to manifest parent.",
    )
    ...
    parser.add_argument("--query-model", default=DEFAULT_DMX_MODEL, help="Model used for grounding query generation.")
    parser.add_argument("--query-api-url", default=DMX_API_URL, help="Chat completions endpoint for grounding LLM calls.")
    ...
    parser.add_argument(
        ...
        help="Local sentence-transformers model path for dense retrieval.",
    )
    parser.add_argument(
        "--query-prefix",
    ...
    parser.add_argument(
        "--reranker-model-path",
        default=str(DEFAULT_RERANKER_MODEL_PATH),
        help="Local cross-encoder model path for paragraph reranking.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use DMX to refine final grounded matches with local paragraph context from the source paper.",
    )
    parser.add_argument(
...
def load_api_key(env_path: Path, *, key_name: str) -> str:
    ...
    api_key = normalize_whitespace(
        env_values.get(key_name)
        or env_values.get("DMX_API_KEY")
...
def run_grounding(args: argparse.Namespace) -> dict[str, Any]:
    ...
    reranker_model_path = Path(args.reranker_model_path).expanduser().resolve()
    ...
    refinement_generator: DmxQueryGenerator | None = None
    paper_contexts: dict[str, PaperContextRecord] = {}
    ...
    if refinement_requested:
        try:
            refinement_generator = DmxQueryGenerator(
                QueryGeneratorConfig(
                    api_url=args.query_api_url,
                    model=args.query_model,
                    env_path=Path(args.env),
    ...
    return {
        ...
        "retrieval": {
            ...
            "reranker_model_path": None if args.disable_reranker else str(reranker_model_path),
