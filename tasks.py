...
from schemas import (
    DemoRequest,
    TASK_AUTHOR_RESEARCH_PROFILE,
    TASK_IDEA_GENERATION,
    TASK_PAPER_GROUNDING_COMPARE,
...
def _run_manifest(
    *,
    ...
    from local_pipelines import pdf_xml_pipeline
    ...
    args = pdf_xml_pipeline.build_parser().parse_args(cli_args)
    manifest = pdf_xml_pipeline.run_pipeline(args)
    return manifest, Path(manifest["manifest_path"]).resolve()


def _default_query_api_url(env_path: Path) -> str | None:
    env_values = load_env_values(env_path)
    base_url = get_env_value(env_values, "GROUNDING_API_URL")
    if base_url:
        return base_url
...
def _run_grounding(
    *,
    ...
    from local_pipelines import grounding_pipeline
    ...
    embedding_model_path = normalize_whitespace(params.get("embedding_model_path"))
    if embedding_model_path:
        cli_args.extend(["--embedding-model-path", embedding_model_path])
    reranker_model_path = normalize_whitespace(params.get("reranker_model_path"))
    if reranker_model_path:
        cli_args.extend(["--reranker-model-path", reranker_model_path])
    ...
    args = grounding_pipeline.build_parser().parse_args(cli_args)
    payload = grounding_pipeline.run_grounding(args)
...
def _execute_topic_trend_review(request: DemoRequest, run_dir: Path, client: KG2ApiClient) -> dict[str, Any]:
    params = merge_task_params(request.task_type, request.params)
...
def _execute_related_authors(request: DemoRequest, run_dir: Path, client: KG2ApiClient) -> dict[str, Any]:
    ...
    summary = (
        f"Returned {len(authors)} related authors using KG2API author recall."
        if authors
...
def _execute_author_profile(request: DemoRequest, run_dir: Path, client: KG2ApiClient) -> dict[str, Any]:
    params = merge_task_params(request.task_type, request.params)
    author_name = normalize_whitespace(request.input_payload.get("author_name"))
    if not author_name:
        raise ValueError("Task 4 requires author_name.")
...
def _execute_idea_generation(request: DemoRequest, run_dir: Path, client: KG2ApiClient) -> dict[str, Any]:
    params = merge_task_params(request.task_type, request.params)
    topic_text = normalize_whitespace(request.input_payload.get("topic_text") or request.input_payload.get("idea_text"))
    if not topic_text:
...
def execute_request(request: DemoRequest, run_dir: Path) -> dict[str, Any]:
    settings = load_kg2api_settings(request.env_path, request.params)
    with KG2ApiClient(settings) as client:
        if request.task_type == TASK_PAPER_GROUNDING_COMPARE:
            response = _execute_grounding_compare(request, run_dir, client)
        elif request.task_type == TASK_TOPIC_TREND_REVIEW:
            response = _execute_topic_trend_review(request, run_dir, client)
        elif request.task_type == TASK_RELATED_AUTHORS:
            response = _execute_related_authors(request, run_dir, client)
        elif request.task_type == TASK_AUTHOR_RESEARCH_PROFILE:
            response = _execute_author_profile(request, run_dir, client)
        elif request.task_type == TASK_IDEA_GENERATION:
            response = _execute_idea_generation(request, run_dir, client)
        else:
