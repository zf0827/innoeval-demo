...
class DemoRendererTests(unittest.TestCase):
    ...
    def test_render_grounding_markdown_contains_sections(self) -> None:
        response = {
            "task_type": TASK_PAPER_GROUNDING_COMPARE,
    ...
    def test_render_author_profile_markdown_contains_trajectory(self) -> None:
        response = {
            "task_type": TASK_AUTHOR_RESEARCH_PROFILE,
...
class ConfigAndCliTests(unittest.TestCase):
    def test_load_kg2api_settings_reads_env(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env_path.write_text(
                "KG2API_BASE_URL=http://127.0.0.1:8000\n"
                "KG2API_API_KEY=test-key\n"
                "KG2API_TIMEOUT=42\n",
                encoding="utf-8",
            )
            settings = load_kg2api_settings(env_path)
    ...
    def test_demo_request_defaults(self) -> None:
