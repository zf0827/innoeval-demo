...
from common import normalize_whitespace, truncate_text
from schemas import (
...
def _render_grounding_result(response: dict[str, Any]) -> str:
    ...
    lines = [
        "# Task 1: Paper / Idea Grounding Compare",
        "",
        "## Input",
        "",
...
def _render_author_profile(response: dict[str, Any]) -> str:
    result = response["result"]
    lines = [
        "# Task 4: Author Research Profile",
        "",
        "## Input",
        "",
        *_render_mapping_lines(response["input_summary"]),
...
def render_response_markdown(response: dict[str, Any]) -> str:
    ...
    if task_type == TASK_PAPER_GROUNDING_COMPARE:
    ...
    if task_type == TASK_AUTHOR_RESEARCH_PROFILE:
