...
from common import DEFAULT_ENV_PATH, DEFAULT_RUN_ROOT, normalize_whitespace, read_json, resolve_run_dir, write_json, write_text
from renderers import render_response_markdown
from schemas import DemoRequest, SUPPORTED_TASK_TYPES
from tasks import execute_request


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the open-source InnoEval demo tasks and emit JSON + Markdown results.")
    ...
    parser.add_argument("--idea-text", default=None, help="Idea text input for task 1 or task 3.")
    parser.add_argument("--pdf-path", default=None, help="PDF input for task 1 or task 3.")
    parser.add_argument("--topic-text", default=None, help="Topic text input for task 2 or task 5.")
    parser.add_argument("--author-name", default=None, help="Author name input for task 4.")
    ...
    parser.add_argument("--output-root", default=str(DEFAULT_RUN_ROOT), help="Root folder for demo runs.")
    parser.add_argument("--run-id", default=None, help="Optional run id.")
    parser.add_argument("--env", default=str(DEFAULT_ENV_PATH), help="Path to the demo .env file.")
...
def _build_request_from_args(args: argparse.Namespace) -> DemoRequest:
    ...
    if args.task_type in {"paper_grounding_compare", "related_authors"}:
        if bool(args.idea_text) == bool(args.pdf_path):
            raise ValueError("Task 1 and task 3 require exactly one of --idea-text or --pdf-path")
        if args.idea_text:
        ...
        if not args.topic_text:
            raise ValueError("Task 2 requires --topic-text")
        input_payload["topic_text"] = args.topic_text
    elif args.task_type == "author_research_profile":
        if not args.author_name:
            raise ValueError("Task 4 requires --author-name")
        input_payload["author_name"] = args.author_name
    elif args.task_type == "idea_generation":
        if not args.topic_text:
            raise ValueError("Task 5 requires --topic-text")
        input_payload["topic_text"] = args.topic_text
...
def _build_input_summary_for_run_id(request: DemoRequest) -> str:
