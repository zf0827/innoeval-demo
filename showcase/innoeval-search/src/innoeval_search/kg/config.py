...
def _default_llm_api_url() -> str:
    explicit = os.getenv("SEARCH_LLM_API_URL")
    if explicit:
        return explicit
...
def _default_llm_model() -> str:
    return os.getenv("SEARCH_LLM_MODEL") or os.getenv("OPENAI_MODEL") or DEFAULT_LLM_MODEL


def _default_llm_api_key() -> str | None:
    return (
        os.getenv("SEARCH_LLM_API_KEY")
        or os.getenv("OPENAI_API_KEY")
        or os.getenv("DMX-API-KEY")
...
class SearchConfig:
    ...
    def validate(self) -> None:
        ...
        if not self.llm_api_key:
            raise RuntimeError("LLM API key is required. Set SEARCH_LLM_API_KEY or OPENAI_API_KEY in .env.")
