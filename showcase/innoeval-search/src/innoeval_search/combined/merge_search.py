...
def resolve_llm_api_url(env_values: dict[str, str], explicit_url: str | None = None) -> str:
    ...
    if explicit:
        return explicit
    direct = get_env_value(env_values, "SEARCH_LLM_API_URL")
    if direct:
        return direct
...
def resolve_llm_model(env_values: dict[str, str], explicit_model: str | None = None) -> str:
    ...
    if explicit:
        return explicit
    return get_env_value(env_values, "SEARCH_LLM_MODEL", "OPENAI_MODEL") or DEFAULT_LLM_MODEL


def resolve_llm_api_key(env_values: dict[str, str]) -> str:
    api_key = get_env_value(env_values, "SEARCH_LLM_API_KEY", "OPENAI_API_KEY", "DMX-API-KEY", "DMX_API_KEY")
    if not api_key:
        raise ValueError("No LLM API key found in env file. Set SEARCH_LLM_API_KEY or OPENAI_API_KEY.")
    return api_key
