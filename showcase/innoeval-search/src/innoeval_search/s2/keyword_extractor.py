...
def load_api_key(env_path: Path) -> str:
    ...
    for key_name in ("SEARCH_LLM_API_KEY", "OPENAI_API_KEY", "DMX-API-KEY", "DMX_API_KEY"):
        api_key = values.get(key_name)
        if api_key:
            return api_key

    raise ValueError("No LLM API key found in .env. Set SEARCH_LLM_API_KEY or OPENAI_API_KEY.")
